...
const InstructorDashboard: React.FC<InstructorDashboardProps> = ({ currentUser }) => {
  ...
  const instructorNotifications = useMemo(() => {
  ...
  const markAllAsRead = async () => {
    try {
      const unreadNotifications = instructorNotifications.filter(n => !currentUser.notificationsRead?.[n.id]);
      ...
      // Ensure local notifications state is also updated to reflect read status immediately
  ...
  const unreadCount = useMemo(() => {
    return instructorNotifications.filter(n => !currentUser.notificationsRead?.[n.id]).length;
  }, [instructorNotifications, currentUser]);

  const sortedNotifications = useMemo(() => 
    [...instructorNotifications].sort((a,b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
  , [instructorNotifications]);

  const TABS: { id: TurnUpTab; label: string }[] = [
    { id: 'reservations', label: '레슨 스케줄' },
