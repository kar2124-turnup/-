...
const Instructors: React.FC<InstructorsProps> = ({ onImpersonate }) => {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (!editingUser.id && !password) {
        showToast('오류', '신규 프로의 비밀번호를 입력해주세요.', 'error');
        return;
    }
  ...
  const handleConfirmAction = async (e: React.FormEvent) => {
    ...
    try {
        if (actionToConfirm.type === 'save' && editingUser) {
            ...
            setUsers(updatedUsers);
            showToast('성공', `${userToDelete.name} 프로 정보가 삭제되었습니다.`, 'success');
        }
        handleCancelAction();
    } catch (error) {
  ...
  const getModalContent = () => {
    ...
    if (actionToConfirm.type === 'delete' && actionToConfirm.userToDelete) {
        ...
        return {
            title: '프로 삭제 확인',
            description: `정말로 ${user.name} (${user.username}) 프로를 삭제하시겠습니까?`
        };
    }
          ...
          <motion.div layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <Card className="mb-6">
              <h3 className="text-lg font-bold text-white mb-4">
                {editingUser.id ? `프로 정보 수정: ${editingUser.name}` : '신규 프로 등록'}
              </h3>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3">
                      <Input
                          id="instructorColor"
                          type="color"
                          value={editingUser.color || '#ef4444'}
      ...
      <Card initial={{ opacity: 0, y: 0 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="overflow-x-auto">
          ...
          {instructors.length === 0 && (
              <p className="text-center py-8 text-slate-400">등록된 프로가 없습니다.</p>
          )}
        </div>
      </Card>
